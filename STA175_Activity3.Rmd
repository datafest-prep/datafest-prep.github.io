...
Today, you are going to be working with the energy efficiency data (the same data set we have been working with) as though we were in a competition. If you need a refresher on plotting, look back at Activity 2. If you need a refresher on data wrangling, you can look at [the slides](https://https://datafest-prep.github.io/slides/data_wrangling.html) we covered last time.
...
We will continue using the energy efficiency data. Load the `train.csv` data set into R. The easiest way to do this is just to copy the chunk that you used to load the data in previous activities and paste that into your Markdown file for this activity. You can also go back and look at the activity for loading the data for full instructions. 
...
We need to (1) look at only the residential buildings and then (2) choose only the ones over 600 sq feet. We also need to know the age of the building. 
</DIV>
...
# with at least 600 sq feet
trainNew <- train %>%
  filter( building_class == "Residential" &
            floor_area > 6000)
```
...
Note that we are currently in 2022, so we subtract the year the building was constructed from 2022 to determine building age. 

```{r eval = F}
# Add on a column for age
trainNew <- trainNew %>%
...
Here, we use `is.na` to help us. This is a function that returns "TRUE" if a row has missing data for Age and "FALSE" otherwise. For example, 

```{r eval = F}
# The first row is not missing age
is.na(trainNew$Age[1])
# The 16th row is!
is.na(trainNew$Age[16])
```
To count how many are missing age, we can use `sum`. 

```{r eval = F}
sum(is.na(trainNew$Age))
```
There are 408 buildings with no age information.  
</DIV>
...
To figure this out, we need to (1) create a column indicating which buildings are missing their age, (2) group together the different facility types and (3) find out how many buildings of each facillity type are missing data on age. We can do that using the following

```{r eval = F}
trainNew %>%
  # Add a new column indicating missing age
  mutate(isMissing = is.na(Age)) %>%
  # Group the data by facility type
  group_by(facility_type) %>%
  # Count how many buildings are missing age
  summarise(sum(isMissing))
```

 
From this, we will see that `Multifamily_Uncategorized` buildings have the most missing data (383 buildings). 
</DIV>
...
Feel free to change the color and shape!

```{r eval = F}
ggplot(trainNew, aes(x = Age,
             y = site_eui)) +
  geom_point(col = "blue") +
  labs(x = "Building Age",
       y = "Site EUI",
       title = "Figure 1",
       caption = "A scatter plot for the relationship between building age and site energy use") +
  theme_bw()
```
